...
real_distance =0.75; %unit: m
height = 0.02;       %unit: m
title_time = sprintf('%s distance:%2f m, height: %2f m','Time Domain-',real_distance,height);
c1 = 299792458;
...
if Fig==1
 ...
 figure(6);hold on;grid on;  % 用来保证线性化坐标是一致的。
 xlabel('Delay (s)');
 ylabel('Linear Path Loss (w)');
 title(title_time);
end
...
TOA_dis=sort(ftoa_dist)-0.4;
TOA_Error=TOA_dis-real_distance;
Mean_RSS=mean(RSS)
...
fid = fopen('..\amptitude.txt', 'a+');
fprintf(fid, '%f %f  ', height,real_distance );
...
fid1 = fopen('..\ranging_error.txt', 'a+');
fprintf(fid1, '%f %f  ', height,real_distance );
fprintf(fid1, '%f ', TOA_Error);
fprintf(fid,'%f %f', Mean_dis,Var_dis);
fprintf(fid1, '\n');
fclose(fid1);

% log the file of ranging error
fid2 = fopen('..\ average_summary.txt', 'a+');
fprintf(fid2, '%f %f  ', height,real_distance );
fprintf(fid2, '%f %f %f %f', Mean_FP,Mean_dis,Var_FP,Var_dis);
fprintf(fid2, '\n');
fclose(fid2);
