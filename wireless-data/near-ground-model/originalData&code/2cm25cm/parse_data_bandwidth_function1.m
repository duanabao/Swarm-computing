...
if Centralband~=0 
   ...
   logf=Centralband;
elseif orignal_band_end~=0e9
   B_start=orignal_band_end - Band;
   B_end=B_start+Band;
...
fprintf(fid2, '%f %f %f %f %0.5g %0.5g', Mean_FP,Var_FP,Mean_error,Mean_RSS,Var_error);
fprintf(fid2, '\n');
fclose(fid2);

% log the file of RSS
pathFile= sprintf('..\\%s\\RSS.txt',pathName);
fid3 = fopen(pathFile, 'a+');

fprintf(fid3, '%f %f %d %d ', height,real_distance,logf,Band );
...
annotation('textbox', [.5 .8, .1, .1], 'string', str, 'fontsize',9, ...
     'color', 'm', 'edgecolor', 'none', 'Tag' , 'annotion');
...
figure(5)
cla(h5);
delete(findall(gcf,'Tag','annotion'))
figure(6);
cla(h6);
Band = Band + 0.1e9;    %Slected Bandwidth
if Centralband~=0 
   B_start=Centralband -Band/2; 
   B_end=B_start+Band; 
   pathName = sprintf('centralf%dHz',Centralband);
